...
function CookingCompareInline({
  ...
  const ancientPotRef = useRef<HTMLDivElement>(null);
  const modernPotRef  = useRef<HTMLDivElement>(null);
  ...
  const handleDragEnd = useCallback(
    (_e: MouseEvent | TouchEvent | PointerEvent, info: PanInfo, id: string) => {
      if (inPotIds.includes(id)) return;

      const isOverCircle = (ref: React.RefObject<HTMLDivElement | null>) => {
        if (!ref.current) return false;
        const rect = ref.current.getBoundingClientRect();
        const cx = rect.left + rect.width  / 2;
        const cy = rect.top  + rect.height / 2;
        const r  = rect.width / 2;
        return Math.hypot(info.point.x - cx, info.point.y - cy) < r;
      };

      if (isOverCircle(ancientPotRef) || isOverCircle(modernPotRef)) {
          ...
          >
            {inPotIds.length === 0 && (
              <div style={{
                ...
                color: "rgba(139,90,43,0.25)", fontSize: 11, letterSpacing: 2,
              }}>
                空盘
              </div>
            )}
          ...
          >
            {inPotIds.length === 0 && (
              <div style={{
                position: "absolute", inset: 0,
                display: "flex", alignItems: "center", justifyContent: "center",
                color: "rgba(156,163,175,0.35)", fontSize: 11, letterSpacing: 2,
              }}>
                空盘
              </div>
            )}
        ...
        {finished && (
          <motion.div
            ...
            style={{
              ...
              gap: 12, borderRadius: 10, zIndex: 10, padding: 20,
            }}
          >
        ...
        >
          ...
          <div style={{
            ...
            <span style={{
              ...
              fontSize: 42, color: "rgba(90,59,31,0.85)",
              fontWeight: 600, letterSpacing: 8,
            }}>古</span>

            <div style={{
              width: potSize, height: potSize,
              ...
              {inPotIds.length === 0 && (
                <div style={{
                  position: "absolute", inset: 0,
                  display: "flex", alignItems: "center", justifyContent: "center",
                  color: "rgba(139,90,43,0.25)", fontSize: 13, letterSpacing: 3, zIndex: 1,
                }}>空盘</div>
              )}
              {[...inPotIds].sort((a, b) => a === "soup" ? -1 : b === "soup" ? 1 : 0).map((id) => {
                const idx = ingredients.findIndex((i) => i.id === id);
            ...
            <div style={{
              ...
              maxWidth: potSize + 40, justifyContent: "center",
            }}>
              {ingredients.map((item) => {
                const done = inPotIds.includes(item.id);
                return (
                  ...
                  >
                    <span style={{ display: "flex", alignItems: "center", gap: 5 }}>
                      {(() => { const Icon = INGREDIENT_ICON_MAP[item.id]; return Icon ? <Icon size={13} strokeWidth={2} /> : null; })()}
                      <span>{item.label}</span>
                    </span>
                  </motion.div>
                );
          ...
          <div style={{
            ...
            <span style={{
              ...
              fontSize: 42, color: "rgba(55,65,81,0.9)",
              fontWeight: 600, letterSpacing: 8,
            }}>今</span>
            ...
            <div style={{
              ...
              {inPotIds.length === 0 && (
                <div style={{
                  position: "absolute", inset: 0,
                  display: "flex", alignItems: "center", justifyContent: "center",
                  color: "rgba(156,163,175,0.35)", fontSize: 13, letterSpacing: 3, zIndex: 1,
                }}>空盘</div>
              )}
              {[...inPotIds].sort((a, b) => a === "soup" ? -1 : b === "soup" ? 1 : 0).map((id) => {
                const idx = ingredients.findIndex((i) => i.id === id);
          ...
          <button
            ...
            style={{
              ...
              color: "#5a3b1f", fontSize: 13, letterSpacing: 3,
              cursor: "pointer", fontFamily: "inherit",
            }}
          >
            关闭古今烹饪
            ...
            {finished && (
              <motion.div
                ...
                style={{
                  ...
                  display: "flex", background: "rgba(45,38,32,0.88)",
              ...
              >
                ...
                >
                  ...
                  <h3 style={{ margin: "0 0 8px", fontSize: 22, color: "#5a3b1f", letterSpacing: 4 }}>{dishTitle}</h3>
                  <p style={{ margin: "0 0 24px", fontSize: 14, color: "rgba(90,59,31,0.65)", letterSpacing: 2 }}>八宝入釜 · 古今一味</p>
                  <div style={{ display: "flex", gap: 32, marginBottom: 28 }}>
                    <div style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 6 }}>
                      <img src="/images/ancient/pot/古代.png" alt="古代完成"
                        style={{ width: 200, height: 200, borderRadius: 14, objectFit: "cover", border: "2px solid rgba(139,90,43,0.3)" }} />
                      <span style={{ fontSize: 20, color: "rgba(90,59,31,0.6)", letterSpacing: 2 }}>古</span>
                    </div>
                    <div style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 6 }}>
                      <img src="/images/modern/pot/现代.png" alt="现代完成"
                        style={{ width: 200, height: 200, borderRadius: 14, objectFit: "cover", border: "2px solid rgba(156,163,175,0.4)" }} />
                  ...
                  <button type="button" onClick={reset}
                    style={{
                      ...
                      cursor: "pointer", fontFamily: "inherit", marginRight: 12,
                    }}
                  >再次烹饪</button>
